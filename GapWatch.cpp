...
{ 
	...
	// Initialize scalars
	Sampling = GW_DEFSAMPLING;
	History = GW_DEFHISTORY;
	Epoch = GW_DEFEPOCH;
	GapTol = (float) GW_DEFGAPTOL;
	Consec = GW_DEFCONSEC;
	Channel = GW_DEFCHANNEL;
	Distance = GW_DEFDISTANCE;
	...
	for ( i = 0; i < Channel; ++i ) 
	{
		AmpTol[ i ] = GW_DEFAMPTOL;
		GlobalMax[ i ] = GW_DEFGLOBALMAX;
		for ( j = 0; j < History; ++j ) 
		{
			LocalMax[ i * History + j ] = GW_DEFLOCALMAX;
		}
	}
...
CGapWatch::SetPath(int c, char *f)
{
	ActiveChannel = c;
...
CGapWatch::Modify(int m, int Value)
{
	int i;
	...
	if ( m == GW_IDXCHANNEL ) {
		...
		case GW_IDXSAMPLING	: Sampling	= Value;
		case GW_IDXEPOCH	: Epoch		= Value;
		case GW_IDXCONSEC	: Consec	= Value;
		case GW_IDXDISTANCE	: Distance	= Value;
		case GW_IDXGAPTOL	: GapTol	= (float) Value / (float) SHRT_MAX;
		case GW_IDXUSEPORTS	: UsePorts	= Value;
